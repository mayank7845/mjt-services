...
export const buyNow = async (req, res) => {
    try {
        const { customerId, shippingAddressId, billingAddressId, lineItems, email, note, tags = [], discountCodes = [], acceptAutomaticDiscounts = false, paymentMethod, currency = "INR", } = req.body;
        if (!customerId) {
        ...
        const shippingAddress = addresses.addresses.find((a) => a.id === shippingAddressId);
        const billingAddress = addresses.addresses.find((a) => a.id === billingAddressId);
